...
export class ButtplugService {
    ...
    this.isConnected.subscribe((isConnected) => {
      if (isConnected) {
        this.client.on('deviceadded', async () => {
          //TO-DO: this.notifications.showToast('Client connected', 'success');
  ...
  async disconnect(): Promise<void> {
    ...
    this.listenersRemove();
  //  To-Do: this.notifications.showToast('Client disconnected.', 'error');
  ...
  private async sendCommand(
    ...
    switch (command) {
      ...
      case 'vibrate':
        ...
        break;
      case 'linear+rotate':
       //To-Do: this.notifications.showToast('Not implemented yet.', 'warning');
        break;
    }
  ...
  private sendCommandErrHandler(e: any): void {
    if (e.toString().includes('does not exist')) {
      // To-Do:  this.notifications.showToast(
